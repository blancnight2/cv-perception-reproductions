...
## 官方指标：TrackEval（HOTA / CLEAR / Identity）

上面那套是自己实现的评测。排行榜主排序指标是 **HOTA**，而 `motmetrics` 不实现它，
于是接了 KITTI 官方所用的 **TrackEval**。KITTI train split 21 段：

| 检测器 | 类别 | **HOTA** | DetA | AssA | MOTA | IDF1 | IDSW |
|---|---|---|---|---|---|---|---|
| YOLO11n@640 | Car | 69.34 | 66.20 | 73.63 | 76.73 | 82.86 | 303 |
| **YOLO11s@960** | **Car** | **76.09** | 73.27 | **79.81** | **83.80** | **88.21** | 211 |
| YOLO11n@640 | Pedestrian | 39.91 | 45.75 | 35.28 | 51.31 | 52.42 | 585 |
| **YOLO11s@960** | **Pedestrian** | **48.33** | 53.31 | 44.30 | 62.68 | 62.93 | 429 |

### 交叉验证抓出了自实现评测的三处协议错误

| | TrackEval 官方 | 本仓库先前的自实现 |
|---|---|---|
| 截断阈值 | **truncation > 0** 即视为 distractor | > 0.5 |
| 25 px 高度过滤 | 作用于**未匹配的检测框** | 错误地作用于**真值** |
| distractor 真值 | **整个移除** | 当作 ignore 区保留 |

自实现对真值过于严苛（保留了官方会剔除的截断目标），Car MOTA 因此偏低约 12 点
（71.90 vs 83.80）、FN 虚高一倍（3211 vs 1454）。**以 TrackEval 为准。**

### HOTA 推翻了先前归因的一半

HOTA 把检测与关联拆成 DetA / AssA：

| 类别 | DetA（检测） | AssA（关联） | 更弱的一半 |
|---|---|---|---|
| Car | 73.27 | **79.81** | **检测** |
| Pedestrian | **53.31** | 44.30 | **关联** |

> 先前「跟踪不是瓶颈、检测召回才是」的结论**只对 Car 成立**。
> 行人的 AssA 44.3 明显低于 DetA 53.3，**关联才是更弱的一半**——
> 行人小、运动不规则、互相遮挡，纯 IoU 关联吃力。
> 先前只看 FN 率与 IDSW 率是**间接指标**：IDSW 低也可能只是因为轨迹本就短
> （行人 Frag 410 / ML 25），根本没机会切换。HOTA 直接把这层遮蔽揭开了。

## 一个被实验否掉的假设：放开 ByteTrack 的低分回收带

ByteTrack 的招牌机制是拿 `(track_low_thresh, track_high_thresh)` 区间的低分框做
第二轮关联（`byte_tracker.py:317`）。而先前用 `conf=0.25` 推理，
**等于在低分框进入跟踪器之前就全部滤掉——那个区间是空的，机制从未生效**。

据此假设「放开它是免费收益」。成对放开 conf 与 `track_low_thresh` 后实测（YOLO11s@960）：

| conf / low | Car HOTA | Car MOTA | Car AssA | Car FN | Car FP | Ped HOTA | Ped MOTA |
|---|---|---|---|---|---|---|---|
| **0.25 / 0.10（基线）** | **76.09** | **83.80** | 79.81 | 1454 | **2234** | **48.33** | **62.68** |
| 0.10 / 0.10 | 75.98 | 83.52 | 79.89 | 1301 | 2461 | 47.83 | 61.16 |
| 0.05 / 0.05 | 76.09 | 83.35 | 80.25 | 1236 | 2569 | 47.57 | 58.69 |
| 0.01 / 0.01 | 75.81 | 82.47 | **80.37** | **1161** | 2853 | 45.59 | 53.80 |

> **假设被否掉。** 漏检确实单调下降（Car 1454→1161，Ped 2798→2267），
> 但误检涨得更多（Car 2234→2853，Ped 919→2431），净值为负：
> Car HOTA 基本持平、MOTA 单调下滑；行人则明显变差（HOTA −2.7、MOTA −8.9）。
>
> **机制本身是有效的**：Car 的 **AssA 单调上升 79.81 → 80.37**，
> 说明低分框确实稳住了关联——只是 DetA 的损失盖过了这点收益。
>
> **真正的结论：ByteTrack 的前提是「低分框大多是被遮挡的真目标」，
> 而在 KITTI 上这个检测器的低分框大多是噪声**，所以这笔交易不划算。
> 另外可见 **MOTA 比 HOTA 对误检敏感得多**（Car MOTA 掉 1.3 而 HOTA 几乎不动）。
