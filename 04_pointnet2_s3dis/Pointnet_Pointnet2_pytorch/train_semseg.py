...
def train_worker_init_fn(worker_id):
    """初始化 DataLoader worker 的随机种子，兼容 Windows spawn 模式。"""
...
def main(args):
    ...
    root = resolve_s3dis_data_root('data/stanford_indoor3d/')
    NUM_CLASSES = 13
    NUM_POINT = args.npoint
    BATCH_SIZE = args.batch_size

    log_string(f'Using S3DIS data root: {root}')
    ...
    try:
        checkpoint = torch.load(str(experiment_dir) + '/checkpoints/model.pth', weights_only=False)
    ...
    best_iou = 0
    try:
        best_checkpoint = torch.load(str(experiment_dir) + '/checkpoints/best_model.pth', weights_only=False)
        if 'class_avg_iou' in best_checkpoint:
            best_iou = best_checkpoint['class_avg_iou']
            log_string('Loaded best_iou: %f' % best_iou)
    except:
        pass
